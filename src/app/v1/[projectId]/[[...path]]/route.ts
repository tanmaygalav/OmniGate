...
async function handleProxy(
  ...
  // 1. Auth Check (Now with CORS Header)
  ...
  // Invalid Key (Now with CORS Header)
  ...
  const projectInfo = keyData.projects as unknown as {
    target_url: string;
    rate_limit: number;
  ...
  // Rate Limit Exceeded (Now with combined Rate Limit + CORS Headers)
  if (!success) {
    return NextResponse.json(
  ...
  try {
    ...
    try {
      await fetch(
        `https://api.europe-west2.gcp.tinybird.co/v0/events?name=gateway_logs`,
        {
          method: "POST",
          headers: {
    ...
    // Bad Gateway Catch (Now with CORS Header)
    return NextResponse.json(
      { error: "Bad Gateway: Could not reach target server" },
